#!/usr/bin/env python3
"""
楽天ROOM投稿完全自動化スクリプト v3
GitHub Actions対応版
毎日10件投稿（異なるカテゴリから選定、商品重複なし）
"""
...
class RakutenAutoPoster:
    def __init__(self):
        ...
        self.posted_products_file = self.data_dir / 'posted_products.json'
        self.load_history()
        self.load_posted_products()
    ...
    def load_posted_products(self):
        """投稿済み商品リストを読み込む"""
        if self.posted_products_file.exists():
            with open(self.posted_products_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.posted_product_hashes: Set[str] = set(data.get("product_hashes", []))
                self.posted_products: List[Dict] = data.get("products", [])
        else:
            self.posted_product_hashes = set()
            self.posted_products = []
    
    def save_posted_products(self):
        """投稿済み商品リストを保存"""
        data = {
            "product_hashes": list(self.posted_product_hashes),
            "products": self.posted_products,
            "last_updated": datetime.now().isoformat()
        }
        with open(self.posted_products_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    
    def generate_product_hash(self, product: Dict) -> str:
        """商品のハッシュを生成（重複チェック用）"""
        product_str = f"{product['name']}_{product['category']}_{product['price']}"
        return hashlib.md5(product_str.encode()).hexdigest()
    
    def is_product_posted(self, product_hash: str) -> bool:
        """商品が投稿済みかチェック"""
        return product_hash in self.posted_product_hashes
    ...
    def generate_product(self, category: Dict, max_attempts: int = 10) -> Dict:
        """商品データを生成（重複チェック付き）"""
        for attempt in range(max_attempts):
            product = {
                "name": f"{category['name']}の人気商品 #{random.randint(1000, 99999)}",
            ...
            # 投稿済みでなければ返す
            if not self.is_product_posted(product_hash):
                product["hash"] = product_hash
                return product
        
        # max_attempts回試してもダメな場合はタイムスタンプを追加して強制生成
        product["name"] += f" {datetime.now().timestamp()}"
        product["hash"] = self.generate_product_hash(product)
        return product
    ...
    def post_single(self, post_number: int) -> bool:
        ...
        try:
            ...
            # 投稿済み商品として記録
            self.posted_product_hashes.add(product['hash'])
            self.posted_products.append({
                "name": product['name'],
                "category": product['category'],
                "hash": product['hash'],
    ...
    def run(self) -> bool:
        """10件投稿を実行"""
        logger.info("=" * 70)
        logger.info("🎯 楽天ROOM投稿完全自動化エンジン (10件投稿版 - 重複防止機能付き)")
        logger.info(f"⏰ 実行時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S (JST)')}")
        logger.info(f"📊 投稿済み商品数: {len(self.posted_product_hashes)}")
        logger.info("=" * 70 + "\n")
